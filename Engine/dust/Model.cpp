...
void Model::Init(const std::string& directoryPath, const std::string& filename) {
	...
	size_ = static_cast<uint32_t>(modelData_.meshes.size());
...
void Model::Term() {
	...
	for (uint32_t i = 0; i < size_; ++i) {
		// meshDataのdelete
		modelData_.meshes[i].vertexResource.reset();
		modelData_.meshes[i].indexResource.reset();
...
void ModelMethods::LoadObjFileAssimp(const std::string& directoryPath, const std::string& filename) {
	...
	for (uint32_t meshIndex = 0; meshIndex < scene->mNumMeshes; ++meshIndex) {
		...
		// meshデータの保存場所の指定
		auto& meshData = result.meshes[meshIndex]; //<! 現在のmeshのデータの取り出し
		meshData.Create(MyEngine::GetDevicesObj(), mesh->mNumVertices, mesh->mNumFaces);
		...
		for (uint32_t faceIndex = 0; faceIndex < mesh->mNumFaces; ++faceIndex) {
			...
			for (uint32_t element = 0; element < face.mNumIndices; ++element) {
				// データの保存
				meshData.indexResource->operator[](element) = face.mIndices[element];
			}
		}
	}
