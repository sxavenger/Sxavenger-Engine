...
void SkinnedMeshRendererComponent::InputSkinnedMesh::UpdateBottomLevelAS(const DirectXQueueContext* context) {
	StreamLogger::AssertA(isCreateMesh, "mesh is not created.");
...
void SkinnedMeshRendererComponent::CreateMesh(const Uuid& referenceMesh) {
	...
	mesh_.Create(System::GetDirectQueueContext(), referenceMesh_.Get());
	CreateCluster();
}
...
void SkinnedMeshRendererComponent::Skinning() {
	...
	Graphics::SetSkinningPipeline(System::GetDirectQueueContext());
	...
	Graphics::DispatchSkinning(System::GetDirectQueueContext(), parameter, cluster_.info->At(0));
}
...
void SkinnedMeshRendererComponent::CreateCluster() {
	...
	cluster_.info->Create(System::GetDxDevice(), 1);
	cluster_.info->At(0) = kVertexSize;

	//* influence
	cluster_.influence = std::make_unique<DimensionBuffer<VertexInfluence>>();
	cluster_.influence->Create(System::GetDxDevice(), kVertexSize);
	...
	cluster_.palette->Create(System::GetDxDevice(), static_cast<uint32_t>(kJointSize));
...
const ArmatureComponent* SkinnedMeshRendererComponent::GetArmatureComponent() const {
	...
	const BehaviourAddress& child = BaseComponent::RequireParent()->FindChild(ArmatureComponent::kArmatureName);
