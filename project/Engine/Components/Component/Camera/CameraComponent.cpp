...
void CameraComponent::CreateBuffer() {
	for (size_t i = 0; i < buffers_.size(); ++i) {
		...
		buffers_[i]->Create(System::GetDxDevice());
		buffers_[i]->At().Init();
	}
}
...
void CameraComponent::PushLineFrustum() {
	...
	for (uint8_t i = 0; i < 4; ++i) {
		...
		Graphics::PushLine(frustumPoint[nearIndex], frustumPoint[nearNext], color, 0.6f);
		Graphics::PushLine(frustumPoint[farIndex],  frustumPoint[farNext],  color, 0.6f);
		Graphics::PushLine(frustumPoint[nearIndex], frustumPoint[farIndex], color, 0.6f);
	}
}
