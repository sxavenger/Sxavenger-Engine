...
void DebugPrimitive::DrawToScene(const DirectXQueueContext* context, const D3D12_GPU_VIRTUAL_ADDRESS& camera) {
...
void DebugPrimitive::PushGrid(const Matrix4x4& viewInv, const Matrix4x4& projInv, const Vector2f& size, float radius) {

	Vector3f positions[2] = {};

	for (size_t i = 0; i < 2; ++i) {
		Vector3f target = Matrix4x4::Transform({ 0.0f, 0.0f, static_cast<float>(i) }, projInv);
		positions[i]    = Matrix4x4::Transform(target, viewInv);
	}
	...
	const Vector3f position = Matrix4x4::GetTranslation(viewInv);
...
void DebugPrimitive::CreatePipeline() {
...

	{
		...
		desc.SetRasterizer(D3D12_CULL_MODE_NONE, D3D12_FILL_MODE_SOLID);
		desc.SetPrimitive(DxObject::PrimitiveType::PointList);
