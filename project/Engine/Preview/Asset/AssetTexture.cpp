...
void AssetTexture::Setup(const DirectXQueueContext* context, const DirectX::ScratchImage& image) {
	...
	BaseAsset::Complete();
	Logger::EngineThreadLog(std::format("[AssetTexture]: texture setup complete. uuid: {}", BaseAsset::GetId().Serialize()));
}
...
ComPtr<ID3D12Resource> AssetTexture::CreateTextureResource(const DirectX::TexMetadata& metadata) const {
	// propの設定
...
ComPtr<ID3D12Resource> AssetTexture::UploadTextureData(const DirectXQueueContext* context, ID3D12Resource* texture, const DirectX::ScratchImage& image) const {
