...
void BaseDebugPrimitive::Draw(const DirectXThreadContext* context, const ACameraActor* camera) {
	// commandListの取得
...
void DebugPrimitive::DrawToScene(const DirectXThreadContext* context, const ACameraActor* camera) {
...
void DebugPrimitive::CreatePipeline() {
	...
	pipeline_->CreateBlob(kPackagesShaderDirectory / "debugPrimitive/debugPrimitive.vs.hlsl", GraphicsShaderType::vs);
	pipeline_->CreateBlob(kPackagesShaderDirectory / "debugPrimitive/debugPrimitive.ps.hlsl", GraphicsShaderType::ps);
