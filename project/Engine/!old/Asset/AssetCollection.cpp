...
AssetCollection::FileData& AssetCollection::GetFile(const std::filesystem::path& filepath) {
...
std::shared_ptr<AssetUnknown> AssetCollection::LoadUnknown(FileData& fileData, const std::filesystem::path& filepath) {
	auto& [type, file] = fileData;
	Assert(type == FileType::Unknown, "different type.");

	auto asset = std::make_shared<AssetUnknown>();
	file       = asset;
...
std::shared_ptr<AssetModel> AssetCollection::LoadModel(FileData& fileData, const std::filesystem::path& filepath) {
	...
	if (std::holds_alternative<std::pair<std::shared_ptr<AssetModel>, std::shared_ptr<AssetAnimator>>>(file)) { //!< modelまたはanimatorが存在する場合
		...
		AsyncAssetThreadCollection::PushTask(model);
		return model;
...
std::shared_ptr<AssetAnimator> AssetCollection::LoadAnimator(FileData& fileData, const std::filesystem::path& filepath) {
	...
	if (std::holds_alternative<std::pair<std::shared_ptr<AssetModel>, std::shared_ptr<AssetAnimator>>>(file)) { //!< modelまたはanimatorが存在する場合
		...
		AsyncAssetThreadCollection::PushTask(animator);
		return animator;
...
std::shared_ptr<AssetUnknown> AssetCollection::TryLoadUnknown(FileData& fileData, const std::filesystem::path& filepath) {
	...
	if (std::holds_alternative<std::shared_ptr<AssetUnknown>>(file)) { //!< unknownが存在する場合
		return std::get<std::shared_ptr<AssetUnknown>>(file);
	}
