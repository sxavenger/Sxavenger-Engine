...
void main(
	...
	if (groupThreadId < m.vertexCount) {
		...
		output.position = mul(input.position, mul(gTransform[instanceIndex].mat, viewProj));
		output.worldPos = mul(input.position, gTransform[instanceIndex].mat).xyz;
		output.texcoord = mul(float4(input.texcoord, 0.0f, 1.0f), gUVTransform.mat).xy;
		output.normal   = normalize(mul(input.normal, (float3x3)gTransform[instanceIndex].matInverseTransopse));
