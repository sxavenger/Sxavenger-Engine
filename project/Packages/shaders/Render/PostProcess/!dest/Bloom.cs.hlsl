...
void main(uint3 dispathThreadId : SV_DispatchThreadID) {
	...
	//		float weight = Gaussian2d(float2(x, y), radius + 1.0f);
	...
	for (int x = -kSamples.x; x <= kSamples.x; ++x) {
		int2 sample_index = int2(index) + int2(x, 0);
		...
		float radius = length(float2(x, 0));
			
		float weight = Gaussian2d(float2(x, 0), radius + 1.0f);
	...
	for (int y = -kSamples.y; y <= kSamples.y; ++y) {
		int2 sample_index = int2(index) + int2(0, y);
		...
		float radius = length(float2(0, y));
			
		float weight = Gaussian2d(float2(0, y), radius + 1.0f);
...
// todo: bloomの形をtextureで指定できるpost effectの追加
