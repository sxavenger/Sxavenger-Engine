...
/*
# reference
 - https://github.com/EpicGames/UnrealEngine/blob/46544fa5e0aa9e6740c19b44b0628b72e7bbd5ce/Engine/Shaders/Private/ACES.ush
*/
...
namespace ACES {
	...
	//! @brief Convert [AP0(ACES) to AP1(OCES)] color space
	//! @param  AP0(ACES) color space
	//! @return AP1(OCES) color space
	...
	//! @brief Convert [AP1(OCES) to sRGB] color space
...
/*
-----------------------------------------------------------------------------
 ACES Compute Shader for DirectX12
-----------------------------------------------------------------------------
 - Input:  gInputTexture  (DXGI_FORMAT_R32G32B32A32_FLOAT)
 - Output: gOutputTexture (DXGI_FORMAT_R32G32B32A32_FLOAT)
 - Tonemapping: ACES RRT+ODT (sRGB, Rec.2020 HDR 1000nits, P3 D60 HDR 2000nits)
 - Output is always linear (sRGB conversion handled in RTV pipeline, not here)
 - Select ODT by uint gTonemapType
   0: sRGB, 1: Rec.2020 1000nits, 2: P3 D60 2000nits
-----------------------------------------------------------------------------
 Reference: ACES.ush (Epic Games, Inc.)
-----------------------------------------------------------------------------

#include "ACES_Params.hlsl" // Contains all constant matrixes and spline params from ACES.ush

Texture2D<float4> gInputTexture : register(t0);
RWTexture2D<float4> gOutputTexture : register(u0);

cbuffer ACESCB : register(b0)
{
    uint gTonemapType; // 0: sRGB, 1: Rec.2020 1000nits, 2: P3 D60 2000nits
    uint2 gTextureSize;
};
...
void main(uint3 DTid : SV_DispatchThreadID)
{
    ...
    float4 inColor = gInputTexture[DTid.xy];

    float3 aces = inColor.rgb;
    ...
    if (gTonemapType == 0) {
         sRGB display ODT (D65, 48nits, Rec709 primaries)
        outputColor = ODT_sRGB_D65(oces);
    }
    else if (gTonemapType == 1) {
         Rec.2020 HDR display (D65, 1000nits)
        outputColor = ODT_1000nits(oces);
    }
    else if (gTonemapType == 2) {
         P3 D60 HDR display (2000nits)
        outputColor = ODT_2000nits(oces);
    }
    else {
         Default: sRGB
        outputColor = ODT_sRGB_D65(oces);
     ...
     Clamp to [0,1] for safety, leave alpha unchanged
    gOutputTexture[DTid.xy] = float4(saturate(outputColor), inColor.a);
}
*/
